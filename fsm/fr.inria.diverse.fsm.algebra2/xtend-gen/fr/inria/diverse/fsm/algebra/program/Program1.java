...
public class Program1 {
  ...
  public <T extends Object, S extends Object, F extends Object, IS extends S, FS extends S> F make(final FSMAlgebra<T, S, F, IS, FS> concreteAlgebra, final Class<F> fsmClass, final Class<T> transitionClass, final Class<S> stateClass, final Class<IS> initialStateClass, final Class<FS> finalStateClass) {
    F _xblockexpression = null;
    {
      final FSM exp = this.createModel();
      final DeferProxy<T, S, F, IS, FS> df = new DeferProxy<T, S, F, IS, FS>(concreteAlgebra, fsmClass, transitionClass, stateClass, initialStateClass, finalStateClass);
  ...
  public void execute() {
    ...
    InputOutput.<String>println(_evalGraph);
    LinkedList<String> _newLinkedList = CollectionLiterals.<String>newLinkedList("a", "b", "a", "a", "b");
    ExecutableFSMAlgebra _executableFSMAlgebra = new ExecutableFSMAlgebra(_newLinkedList);
    ExecutableExp _make = this.<ExecutableTransitionExp, ExecutableStateExp, ExecutableExp, ExecutableStateExp, ExecutableStateExp>make(_executableFSMAlgebra, ExecutableExp.class, 
      ExecutableTransitionExp.class, ExecutableStateExp.class, ExecutableStateExp.class, 
      ExecutableStateExp.class);
    _make.execute();
  }
}
