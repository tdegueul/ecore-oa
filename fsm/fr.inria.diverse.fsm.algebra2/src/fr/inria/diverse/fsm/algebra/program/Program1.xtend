...
class Program1 {
	...
	def <T, S, F, IS extends S, FS extends S> F make(FSMAlgebra<T, S, F, IS, FS> concreteAlgebra, Class<F> fsmClass,
		...
		val df = new DeferProxy(concreteAlgebra, fsmClass, transitionClass, stateClass, initialStateClass,
			finalStateClass)
		df.fsm(exp)
	}
